...
function set_globals {
  ...
  REPOLOCS[7]="${SHOTCUT_REPO:-https://github.com/mltframework/shotcut.git}"
...
#!/bin/sh
# Set up environment
# Run this instead of trying to run bin/editlux. It runs Editlux with the correct environment.
CURRENT_DIR=\$(readlink -f "\$0")
INSTALL_DIR=\$(dirname "\$CURRENT_DIR")
export LD_LIBRARY_PATH="\$INSTALL_DIR/lib":\$LD_LIBRARY_PATH
export MLT_REPOSITORY="\$INSTALL_DIR/lib/mlt-7"
...
End-of-shotcut-wrapper
  ...
  chmod 755 $TMPFILE || die "Unable to make wrapper script executable"
  cp $TMPFILE "$FINAL_INSTALL_DIR/editlux" || die "Unable to create wrapper script - cp failed"
  ...
  if [ "$ACTION_ARCHIVE" = "1" ]; then
    ...
    tarball="$INSTALL_DIR/editlux.txz"
