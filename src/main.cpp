...
static bool isVulkanDrawMethod(int argc, char **argv, QString *detail)
{
    ...
    if (appDataDir.isEmpty()) {
        QSettings settings(QSettings::NativeFormat,
                           QSettings::UserScope,
                           QStringLiteral("Editlux"),
                           QStringLiteral("Editlux"));
        appDataDir = settings.value(QStringLiteral("appdatadir")).toString();
        if (appDataDir.isEmpty() || !QFile::exists(appDataDir + QStringLiteral("/editlux.ini"))) {
            const int value
    ...
    QSettings settings(appDataDir + QStringLiteral("/editlux.ini"), QSettings::IniFormat);
...
class EditluxTranslator : public QTranslator
{
public:
    ...
    {
        QString translated = QTranslator::translate(context, sourceText, disambiguation, n);
        if (translated.isEmpty() && QString::fromUtf8(sourceText).contains("Shotcut"))
            translated = QString::fromUtf8(sourceText);
        return translated.replace(QStringLiteral("Shotcut"), QStringLiteral("Editlux"));
    }
};
...
public:
    ...
    EditluxTranslator shotcutTranslator;
...
#include <winbase.h>
        ...
        setOrganizationName("Editlux");
#if defined(Q_OS_UNIX) && !defined(Q_OS_MAC)
        setOrganizationDomain("editlux.local");
        setDesktopFileName("io.github.editlux.Editlux");
#else
        setOrganizationDomain("editlux.local");
#endif
        setApplicationName("Editlux");
        ...
        QCommandLineOption fullscreenOption(
            "fullscreen",
            QCoreApplication::translate("main", "Fill the screen with the Editlux window."));
        ...
        QCommandLineOption
            glaxnimateOption("glaxnimate",
                             QCoreApplication::translate("main",
                                                         "Run Glaxnimate instead of Editlux."));
        ...
        if (!dir.exists())
            dir.mkpath(dir.path());
        const auto logFileName = dir.filePath("editlux-log.txt");
        if (QFile::exists(logFileName)) {
            const auto previousLogName = dir.filePath("editlux-log.bak");
        ...
        if (qtBaseTranslator.load("qtbase_" + locale,
            ...
            installTranslator(&qtBaseTranslator);
        shotcutTranslator.load("shotcut_" + Settings.language(), dir.absolutePath());
        ...
        if (event->type() == QEvent::FileOpen) {
            ...
            if (mainWindow) {
                ...
                for (int i = 0; i < 15 && !sent; ++i) {
                    QLocalSocket socket;
                    socket.connectToServer("editlux-file-open");
...
int main(int argc, char **argv)
{
    ...
    if (::qEnvironmentVariableIsSet(kWatchdogEnvVar)) {
        QSplashScreen splash(QPixmap(":/icons/editlux-logo-320x320.png"));

        // Log some basic info.
        LOG_INFO() << "Starting Editlux version" << SHOTCUT_VERSION;
#if defined(Q_OS_WIN)
        ...
        QLocalServer::removeServer("editlux-file-open");
        fileOpenServer.listen("editlux-file-open");
