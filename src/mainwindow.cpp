    ...
    , m_keyframesDock(0)
{
    ...
    setupEditluxToolbar();
#ifdef EDITLUX_OFFLINE
    delete ui->actionFAQ;
    delete ui->actionTutorials;
    delete ui->actionForum;
#endif
    setupOpenOtherMenu();
    setupMenuFile();
    setupMenuView();
...
void MainWindow::setupEditluxToolbar()
{
    ...
    for (auto *action : ui->mainToolBar->actions()) {
        if (action->shortcut() == QKeySequence(QStringLiteral("Ctrl+Z")))
            undoAction = action;
        else if (action->shortcut() == QKeySequence(QStringLiteral("Ctrl+Shift+Z"))
                 || action->shortcut() == QKeySequence(QStringLiteral("Ctrl+Y")))
            redoAction = action;
    }
    ...
    addCategory(tr("Text"),
                QIcon::fromTheme("insert-text",
                                 QIcon(":/icons/oxygen/32x32/actions/insert-text.png")),
                [this, showLeftDock]() {
                    m_elementsDock->selectPage(ElementsDock::Page::Text);
                    showLeftDock(m_elementsDock);
                });
    addCategory(tr("Elements"),
                QIcon::fromTheme("insert-image",
                                 QIcon(":/icons/oxygen/32x32/actions/insert-image.png")),
                [this, showLeftDock]() {
                    m_elementsDock->selectPage(ElementsDock::Page::Graphics);
                    showLeftDock(m_elementsDock);
                });
    ...
    addCategory(tr("Transitions"),
                QIcon::fromTheme("insert-link",
                                 QIcon(":/icons/oxygen/32x32/actions/insert-link.png")),
                [this, showLeftDock]() {
                    m_elementsDock->selectPage(ElementsDock::Page::Transitions);
                    showLeftDock(m_elementsDock);
                });
...
void MainWindow::on_actionAbout_Shotcut_triggered()
{
    const auto copyright = QStringLiteral(
        "Editlux modifications &copy; 2026 Editlux contributors.<br>"
        "Shotcut foundation &copy; 2011-2026 "
        "<a href=\"https://www.meltytech.com/\">Meltytech</a>, LLC");
    const auto license = QStringLiteral(
        "<a href=\"https://www.gnu.org/licenses/gpl.html\">GNU General Public License v3.0</a>");
    const auto url = QStringLiteral("https://github.com/mltframework/shotcut");
    QMessageBox::about(
        this,
        tr("About %1").arg(qApp->applicationName()),
        QStringLiteral(
            "<h1>Editlux version %2</h1>"
            "<p>%1 is a native, offline, open-source video editor for Linux.</p>"
            "<p>Editlux is an independent fork based on "
            "<a href=\"%3\">Shotcut</a>. It is not affiliated with CapCut or ByteDance.</p>"
            "<small><p>%4</p>"
            ...
            "program</li>"
            "</ul></p>"
            "<p>The original Shotcut source and license are available from "
            "<a href=\"%3\">%3</a>. Editlux distributions must include their corresponding "
            "GPL source code.</p>"
            "This program is distributed in the hope that it will be useful, "
            "but WITHOUT ANY WARRANTY; without even the implied warranty of "
...
{
    QMenu *menu = topMenu->addMenu(tr("Editlux Creator Presets"));
...
void MainWindow::resizeEvent(QResizeEvent *event)
{
    ...
    adjustMainToolbar();
    auto *propertiesDock = findChild<QDockWidget *>(QStringLiteral("propertiesDock"));
    auto *playlistDock = findChild<QDockWidget *>(QStringLiteral("PlaylistDock"));
    if (width() < 1280 && propertiesDock && playlistDock && propertiesDock->isVisible()
...
{
    // Editlux uses a fixed creator-oriented toolbar assembled by setupEditluxToolbar().
    // Do not reinsert the legacy Shotcut actions when the window is resized.
}
